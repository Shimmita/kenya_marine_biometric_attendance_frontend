...
const blankStation = { name: '', lat: '', lng: '', radiusMeters: 100, active: true };
const blankTheme = {
  name: '',
  primaryColor: '#0A3D62',
    ...
    <Box sx={{ p: { xs: 0, md: 1 } }}>
      <Stack spacing={2}>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="flex-end" spacing={1}>
          <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('all')}>
            Reset all defaults
          </Button>
        </Stack>

        <Tabs value={tab} onChange={(e, v) => setTab(v)} variant="scrollable" scrollButtons="auto">
          <Tab icon={<Palette />} iconPosition="start" label="Branding" />
          <Tab icon={<Business />} iconPosition="start" label="Stations" />
          <Tab icon={<Schedule />} iconPosition="start" label="Attendance" />
          <Tab icon={<Tune />} iconPosition="start" label="Dropdowns" />
            ...
            <Stack spacing={2.5}>
              ...
              <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: '1px solid rgba(148,163,184,0.2)' }}>
                ...
                <Grid container spacing={1.5} alignItems="center">
                  ...
                  <Grid item xs={12} md={2}><Button startIcon={<AddIcon />} variant="outlined" onClick={handleCreateTheme} fullWidth sx={{ minHeight: 56 }}>Create</Button></Grid>
                </Grid>
              </Paper>

              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                <Button startIcon={<Save />} variant="contained" onClick={() => savePatch({ branding: config.branding, logoUrl: config.logoUrl, themes: config.themes, activeThemeName: config.activeThemeName }, 'Branding and theme saved')}>Save branding/theme</Button>
                <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('themes')}>Reset themes</Button>
                <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('branding')}>Reset branding</Button>
              </Stack>
            </Stack>
          </Paper>
        )}
          ...
          <Paper sx={sectionSx} elevation={0}>
            <Stack spacing={2}>
              <Grid container spacing={1.5}>
                ...
                <Grid item xs={6} md={3}><Button variant="contained" startIcon={<AddIcon />} onClick={handleAddStation} fullWidth sx={{ height: 56 }}>Add station</Button></Grid>
              </Grid>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                <Button startIcon={<Save />} variant="contained" onClick={() => savePatch({ stations: config.stations }, 'Stations saved')}>Save stations</Button>
                <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('stations')}>Reset stations</Button>
              </Stack>
            </Stack>
          </Paper>
        )}
        ...
        {tab === 2 && (
          <Paper sx={sectionSx} elevation={0}>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={4}><FormControlLabel control={<Switch checked={config.geofence?.enabled || false} onChange={(e) => setConfig({ ...config, geofence: { ...config.geofence, enabled: e.target.checked } })} />} label="Enable geofence enforcement" /></Grid>
              <Grid item xs={12} md={4}><FormControlLabel control={<Switch checked={config.attendancePolicy?.allowClockOutsideStation || false} onChange={(e) => setConfig({ ...config, attendancePolicy: { ...config.attendancePolicy, allowClockOutsideStation: e.target.checked } })} />} label="Allow clocking outside station" /></Grid>
              <Grid item xs={12} md={4}><FormControlLabel control={<Switch checked={config.attendancePolicy?.requireBiometricVerification !== false} onChange={(e) => setConfig({ ...config, attendancePolicy: { ...config.attendancePolicy, requireBiometricVerification: e.target.checked } })} />} label="Require biometric verification" /></Grid>
              ...
              <Grid item xs={12}>
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                  <Button startIcon={<Save />} variant="contained" onClick={() => savePatch({ attendancePolicy: config.attendancePolicy, geofence: config.geofence, notificationReminders: config.notificationReminders }, 'Attendance settings saved')}>Save attendance settings</Button>
                  <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('attendancePolicy')}>Reset attendance</Button>
                  <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('notificationReminders')}>Reset reminders</Button>
                </Stack>
              </Grid>
            </Grid>
          </Paper>
          ...
          <Paper sx={sectionSx} elevation={0}>
            <Stack spacing={2}>
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                ...
                <Button variant="outlined" startIcon={<AddIcon />} onClick={handleAddDropdownValue}>Add</Button>
              </Stack>
              ...
              />
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                <Button startIcon={<Save />} variant="contained" onClick={() => savePatch({ dropdowns: config.dropdowns }, 'Dropdown settings saved')}>Save dropdowns</Button>
                <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('dropdowns')}>Reset dropdowns</Button>
              </Stack>
            </Stack>
          </Paper>
        )}
          ...
          <Paper sx={sectionSx} elevation={0}>
            <Stack spacing={2}>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                ...
                <Button variant="contained" startIcon={<AddIcon />} onClick={handleAddDept}>Add</Button>
              </Stack>
              <List dense>
              ...
              <Button startIcon={<RestartAlt />} color="warning" variant="outlined" onClick={() => resetConfig('departments')}>Reset departments</Button>
            </Stack>
          </Paper>
        )}
